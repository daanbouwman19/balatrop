...
export class GameActive {
  ...
  enterState(state: string): void {
    ...
    if (state === "SELECT_CARDS") {
      if (this.submitsRemaining > 0) {
        ...
        this.entities = this.entities.filter(
          (entity) => !(entity instanceof ButtonEntity),
        );
        ...
        const button = new ButtonEntity(
          ...
          () => {
            const selectedCards = this.entities.filter(
              (entity): entity is CardEntity =>
                entity instanceof CardEntity && entity.selected,
            );
    ...
    if (state === "ADD_DAMAGE") {
      this.attack_queue = this.entities.filter(
        (entity): entity is CardEntity =>
          entity instanceof CardEntity && entity.selected,
      );
      this.cardTypes = {};
    ...
    if (state === "GAME_OVER") {
      ...
      const gameOverEntity = new GameOverEntity(
        this.screen.width / 2,
    ...
    if (this.STATE === "ADD_DAMAGE") {
      if (this.actionTimer >= 10) {
        ...
        if (this.subStep === 0) {
          ...
          const damage =
            currentCard.value *
            this.attack_queue[0].calculateTypeMultiplier(
  ...
  isClickable(entity: Entity): entity is Entity & {
    ...
    return (
      "handleClick" in entity &&
      typeof (entity as Record<string, unknown>).handleClick === "function"
    );
  }

  isKeydownListener(
    entity: Entity,
  ): entity is Entity & { keydown: (event: KeyboardEvent) => void } {
    return (
      "keydown" in entity &&
      typeof (entity as Record<string, unknown>).keydown === "function"
    );
  }
}
