...
export class CardEntity extends Entity {
  ...
  selected: boolean;
  lt: number;
  animation: (screen: Screen, t: number, lt: number, dt: number) => void;
  hasSpawnedBlood: boolean;
  ...
  constructor(x: number, y: number, card: PokemonCard) {
    ...
    this.lt = 0;
    this.hasSpawnedBlood = false;

    const handSize = 8;
    this.animation = (screen, t, _lt, dt) => {
      ...
      if (!this.selected) {
        if (this.game) {
          ...
          if (selectedCards.length < 5) {
            ...
            console.log(
              "You can only select up to 5 Pokémon at the same time!",
            );
          }
        }
      } else {
  ...
  attack(
    ...
    this.lt = 0;
    this.hasSpawnedBlood = false;
    this.animation = (_screen, _t, lt, dt) => {
      // Attack animation

      this.x += Math.cos(angle) * 50 * (1 - lt / 20) * dt;
      this.y += Math.sin(angle) * 50 * (1 - lt / 20) * dt;

      this.rotation = angle + Math.PI / 2;

      if (lt >= 20 && !this.hasSpawnedBlood) {
        if (this.game) {
          const part = Particle.blood(targetX, targetY);
          this.game.addEntity(part);
          this.hasSpawnedBlood = true;
        }
      }
  ...
  draw(screen: Screen, _t: number) {
    ...
    this.typeImages.forEach((image, index) => {
      screen
        .c()
        .drawImage(image, halfWidth, this.height + index * image.height * 3.5);
    });
  ...
  update(dt: number) {
    if (this.game) {
      this.lt += dt;
      this.animation(this.game.screen, this.game.t, this.lt, dt);
